...
class System:
    ...
    def __init__(self, nodes: np.ndarray, adj: np.ndarray = None):
        ...
        if adj is None:
            ...
            assert self.adj.shape == (n, n), "Adjacency matrix should be of size NxN and match the number of nodes"

        self.maps = {'+': self._add_edge,
                     '-': self._remove_edge,
    ...
    def apply_maps(self, maps):
        ...
        for mi in maps:
            ...
            assert isinstance(mi[0], str) and mi[0] in self.maps, \
                f"The first element of each map instruction should be a string and a key in the available maps. Found:\
                 {mi[0]}"

            assert isinstance(mi[1], tuple), "the second element in the map instruction is a tuple"
            ...
            expected_args_count = self.maps[mi[0]].__code__.co_argcount - 1  # -1 to exclude 'self'
            actual_args_count = len(mi[1])  # -1 to exclude the map key
            assert actual_args_count == expected_args_count, \
    ...
    def _change_node_property(self, n1: int, y):
        ...
        y = np.array(y)
        assert y.shape[0] == self.nodes.shape[1], "New property dimensions must match the existing property dimensions"
        self.nodes[n1] = y
